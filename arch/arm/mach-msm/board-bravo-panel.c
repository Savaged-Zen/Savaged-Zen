...
static struct msm_lcdc_panel_ops bravo_lcdc_panel_ops = {
	.init		= samsung_oled_panel_init,
	.blank		= samsung_oled_panel_blank,
	.unblank	= samsung_oled_panel_unblank,
};

static struct msm_lcdc_timing bravo_lcdc_timing = {
		.clk_rate		= 24576000,
		.hsync_pulse_width	= 4,
		.hsync_back_porch	= 8,
		.hsync_front_porch	= 8,
...
static struct msm_lcdc_platform_data bravo_lcdc_platform_data = {
	.panel_ops	= &bravo_lcdc_panel_ops,
	.timing		= &bravo_lcdc_timing,
	.fb_id		= 0,
	.fb_data	= &bravo_lcdc_fb_data,
	.fb_resource	= &resources_msm_fb[0],
};

static struct platform_device bravo_lcdc_device = {
	.name	= "msm_mdp_lcdc",
	.id	= -1,
	.dev	= {
		.platform_data = &bravo_lcdc_platform_data,
	},
};
...
static void bravo_brightness_set_work(struct work_struct *work_ptr)
{
	unsigned long flags;
	uint8_t val;
...
int __init bravo_init_panel(void)
{
	...
	INIT_WORK(&brightness_delayed_work, bravo_brightness_set_work);

	ret = platform_device_register(&bravo_lcdc_device);
	if (ret != 0)
		return ret;
